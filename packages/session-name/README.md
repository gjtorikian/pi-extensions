...
| `titleFormat`        | `string`                        | `"{summary} — {dir}"` | Title template. Placeholders: `{summary}` (session name) and `{dir}` (basename of cwd). When the session is unnamed the title is `Pi — {dir}` regardless of format.                                                                             |
