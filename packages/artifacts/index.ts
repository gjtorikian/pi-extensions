...
export default function artifacts(pi: ExtensionAPI) {
  ...
  pi.registerCommand("artifacts", {
    description:
      "Open the artifacts index page in the browser (starts the localhost server if not running)",
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      content: Type.Optional(
        Type.String({ description: "Inline content (markdown or HTML). Alternative to `path`." }),
      ),
      path: Type.Optional(
        Type.String({
          description:
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
      ...
      if (action === "list") {
        ...
        const lines = entries.map((e) => {
          const when = e.mtime
            ? new Date(e.mtime).toISOString().replace("T", " ").slice(0, 19)
            : "";
        ...
        return {
          ...
          details: { action, count: entries.length, serverRunning: port !== null } as Record<
            string,
            unknown
          >,
        };
