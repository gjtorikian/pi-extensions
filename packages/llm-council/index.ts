...
const SPINNER_FRAMES = [
  ...CONFIG.shared.spinner.prefixChars,
  ...[...CONFIG.shared.spinner.prefixChars].reverse(),
];
      ...
      m.status === "done"
        ...
        : m.status === "error"
          ...
          : m.status === "working"
            ? applyColor(
                theme,
                CONFIG.shared.spinner.color,
                SPINNER_FRAMES[spinnerFrame % SPINNER_FRAMES.length],
              )
            : applyColor(
                theme,
                CONFIG.shared.status.waitingIconColor,
                CONFIG.shared.status.waitingIcon,
              );
    lines.push(
      indentLine(
    ...
    render(width: number): string[] {
      ...
      for (const { m, md } of memberMds) {
        ...
        if (m.status === "error") {
          ...
          if (m.error)
            lines.push(
              indentLine(indentLine(applyColor(theme, CONFIG.shared.status.errorColor, m.error))),
            );
        } else {
          lines.push(
            indentLine(
      ...
      if (details.chairman) {
        ...
        if (details.chairman.status === "error" && details.chairman.error) {
          lines.push(
            indentLine(
              indentLine(
                applyColor(theme, CONFIG.shared.status.errorColor, details.chairman.error),
              ),
            ),
          );
        } else if (chairmanMd) {
...
function buildExecArgs(exec: ExecConfig): string[] {
  ...
  if (exec.extensions === null) {
    ...
    for (const name of exec.extensions) {
      args.push(
        "-e",
        path.join(os.homedir(), ".pi", "agent", "extensions", name, "src", "index.ts"),
      );
    }
  }
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      return runCouncil(params.question, ctx.cwd, council, signal, (details) => {
        ...
        const doneCount = details.members.filter(
          ...
          indentLine(
            branchLine(
              applyColor(
                ...
                CONFIG.shared.status.workingLabel,
              ),
              theme,
            ),
          ),
              ...
              details.chairman?.status === "working"
                ...
                : details.chairman?.status === "error"
                  ...
                  : details.chairman?.status === "done"
                    ? applyColor(
                        theme,
                        CONFIG.shared.status.doneColor,
                        CONFIG.shared.status.doneLabel,
                      )
                    : applyColor(
                        theme,
                        CONFIG.shared.status.workingColor,
