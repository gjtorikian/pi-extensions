...
class ChatInput extends CustomEditor {
  ...
  private renderBoxed(stock: string[], contentWidth: number, width: number): string[] {
    ...
    const bottomScroll =
      lastIdx !== -1 && lastIdx !== firstIdx ? getScrollText(stock[lastIdx]!) : null;
    ...
    for (let i = 0; i < stock.length; i++) {
      ...
      body.push(
        this.border(c.side) + pad + prefixStr + pad + stock[i]! + fill + pad + this.border(c.side),
      );
      isFirst = false;
    }
    ...
    if (lastIdx !== -1) {
      for (let i = lastIdx + 1; i < stock.length; i++) {
        ...
        const fill =
          vw + CONFIG.EXTRA_MENU_INDENT < width
            ? " ".repeat(width - vw - CONFIG.EXTRA_MENU_INDENT)
            : "";
  ...
  private renderUnboxed(stock: string[], contentWidth: number, width: number): string[] {
    ...
    const buildTop = (scroll: string | null): string =>
      scroll
        ? this.border(`── ${scroll} `) +
          this.border("─".repeat(Math.max(0, width - visibleWidth(`── ${scroll} `))))
